...
func TestGithubAppIncoming(t *testing.T) {
	...
	verifyIncomingWebhook(t, randomedString, "pipelinerun-incoming", entries, []string{randomedString}, false, false)
}
...
func TestGithubSecondIncoming(t *testing.T) {
	...
	verifyIncomingWebhook(t, randomedString, "pipelinerun-incoming", entries, []string{randomedString}, false, true)
}
...
func TestGithubWebhookIncoming(t *testing.T) {
	...
	verifyIncomingWebhook(t, randomedString, "pipelinerun-incoming", entries, []string{randomedString}, true, false)
}
...
func TestGithubAppIncomingGlobPattern(t *testing.T) {
	...
	verifyIncomingWebhook(t, randomedString, "pipelinerun-incoming", entries, []string{"pac-e2e-ns*"}, false, false)
}
...
func TestGithubAppIncomingGlobPrefixPattern(t *testing.T) {
	...
	verifyIncomingWebhook(t, randomedString, "pipelinerun-incoming", entries, []string{"feature-*"}, false, false)
}
...
func TestGithubAppIncomingNoMatch(t *testing.T) {
	...
	verifyIncomingWebhookNoMatch(t, randomedString, "pipelinerun-incoming", entries, []string{"production-*"}, false, false)
}
...
func TestGithubAppIncomingMultiplePatternsNoMatch(t *testing.T) {
	...
	verifyIncomingWebhookNoMatch(t, randomedString, "pipelinerun-incoming", entries, []string{"production-*", "staging-*", "release-*"}, false, false)
}
...
func TestGithubAppIncomingNoMatchExactName(t *testing.T) {
	...
	verifyIncomingWebhookNoMatch(t, randomedString, "pipelinerun-incoming", entries, []string{"main", "develop", "staging"}, false, false)
}

func verifyIncomingWebhook(t *testing.T, randomedString, pipelinerunName string, entries map[string]string, targets []string, onWebhook, onSecondController bool) {
	...
	assert.Assert(t, httpResp.StatusCode >= 200 && httpResp.StatusCode < 300, "http status Code should be 2xx, got %d", httpResp.StatusCode)
	...
	assert.NilError(t, err)
	assert.Assert(t, len(prsNew.Items) == 1, "Expected 1 PipelineRun, got %d", len(prsNew.Items))
...
// verifyIncomingWebhookNoMatch verifies that an incoming webhook request fails when branch doesn't match targets.
func verifyIncomingWebhookNoMatch(t *testing.T, randomedString, pipelinerunName string, entries map[string]string, targets []string, onWebhook, onSecondController bool) {
	ctx := context.Background()
	ctx, runcnx, opts, ghprovider, err := tgithub.Setup(ctx, onSecondController, onWebhook)
	assert.NilError(t, err)
	...
	title := "TestGithubAppIncomingNoMatch - " + randomedString
	sha, vref, err := tgithub.PushFilesToRef(ctx, ghprovider.Client(), title,
		repoinfo.GetDefaultBranch(),
		targetRefName,
		opts.Organization,
	...
	prsNew, err := runcnx.Clients.Tekton.TektonV1().PipelineRuns(randomedString).List(ctx, metav1.ListOptions{})
	assert.NilError(t, err)
	assert.Assert(t, len(prsNew.Items) == 0, "Expected 0 PipelineRuns when no target matches, got %d", len(prsNew.Items))
