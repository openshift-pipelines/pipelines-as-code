...
func PushFilesToRef(ctx context.Context, client *github.Client, commitMessage, baseBranch, targetRef, owner, repo string, files map[string]string) (*github.Commit, error) {
...
func RunPullRequest(ctx context.Context, t *testing.T, g PRTest) (*params.Run, *ghprovider.Provider, options.E2E, string, string, int, string) {
	...
	commit, err := PushFilesToRef(ctx, ghcnx.Client, g.Label, repoinfo.GetDefaultBranch(), targetRefName,
...
func RunPushRequest(ctx context.Context, t *testing.T, label string, yamlFiles []string, onSecondController, onWebhook bool) (*params.Run, *ghprovider.Provider, options.E2E, string, string, int, string) {
	...
	entries, err := payload.GetEntries(yamlEntries, targetNS, targetBranch, targetEvent, map[string]string{})
	assert.NilError(t, err)

	targetRefName := fmt.Sprintf("refs/heads/%s", targetBranch)
	commit, err := PushFilesToRef(ctx, ghcnx.Client, logmsg, repoinfo.GetDefaultBranch(), targetRefName, opts.Organization, opts.Repo, entries)
	runcnx.Clients.Log.Infof("Commit %s has been created and pushed to %s", commit.GetHTMLURL(), targetRefName)
	sha := commit.GetSHA()
	assert.NilError(t, err)
