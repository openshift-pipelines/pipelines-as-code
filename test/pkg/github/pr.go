...
// PushFilesToExistingBranch pushes files to an existing branch by creating a new commit on top of the parent SHA.
func PushFilesToExistingBranch(ctx context.Context, client *ghlib.Client, commitMessage, parentSHA, targetRef, owner, repo string, files map[string]string) (string, error) {
	...
	// Update the existing ref instead of creating a new one
	ref := &ghlib.Reference{
		Ref: &targetRef,
		Object: &ghlib.GitObject{
			SHA: commit.SHA,
