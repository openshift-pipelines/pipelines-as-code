...
func PushFilesToRefGit(t *testing.T, topts *TestOpts, entries map[string]string, baseRefFrom string) {
	...
	topts.Clients.Clients.Log.Infof("Pushed files to repo %s branch %s", topts.GitHTMLURL, topts.TargetRefName)
}
...
func CreateForkPullRequest(t *testing.T, topts *TestOpts, secondcnx pgitea.Provider,
	...
	assert.NilError(t, err)
	newopts := &TestOpts{
		GitCloneURL: cloneURL, TargetRefName: topts.TargetRefName, Clients: topts.Clients,
	}
	...
	assert.NilError(t, err)
	topts.Clients.Clients.Log.Infof("Created pr %s", pr.HTMLURL)
	return pr
}
