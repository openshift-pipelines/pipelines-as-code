...
func gitlabSetup(ctx context.Context) (*params.Run, E2EOptions, gitlab.Provider, error) {
	...
	if err != nil {
		return nil, E2EOptions{}, gitlab.Provider{}, err
	}
	...
	for _, value := range []string{
		...
		if env := os.Getenv("TEST_GITLAB_" + value); env == "" {
			return nil, E2EOptions{}, gitlab.Provider{}, fmt.Errorf("\"TEST_%s\" env variable is required, cannot continue", value)
		}
	}
	run := &params.Run{}
	if err := run.Clients.NewClients(ctx, &run.Info); err != nil {
