...
export PAC_BITBUCKET_CHECK_SOURCE_IP=true
...
get_info() {
    ...
    [[ ! -s ${tmpfile} ]] &&
        http -f -q --no-verbose --check-status --auth "${user}:${token}" GET ${TEST_BITBUCKET_API_URL}${apipath} \
         -d --output="${tmpfile}"
    jq -r ${typeof} ${tmpfile}    
}
...
function generate() {
    ACCOUNT_ID=$(get_info /user "${TEST_BITBUCKET_USER}" ${TEST_BITBUCKET_TOKEN} .account_id)
    NICKNAME=$(get_info /user "${TEST_BITBUCKET_USER}" ${TEST_BITBUCKET_TOKEN} .nickname)
    MAINBRANCH=$(get_info /repositories/${TEST_BITBUCKET_TEST_REPOSITORY} \
                          "${TEST_BITBUCKET_USER}" ${TEST_BITBUCKET_TOKEN} .mainbranch.name)
    TARGETHASH=$(get_info /repositories/${TEST_BITBUCKET_TEST_REPOSITORY}/commits "${TEST_BITBUCKET_USER}" ${TEST_BITBUCKET_TOKEN} .values[0].hash)    
    cat <<EOF> ${PAYLOAD_FILE}
{
    "repository": {
        ...
        "links": {
            "html": {
                "href": "https://bitbucket.org/${TEST_BITBUCKET_TEST_REPOSITORY}"
            }
        }
    },
    "actor": {
