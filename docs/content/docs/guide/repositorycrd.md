...
(default), `"one_per_pipeline"` and `"disable_all"`.

Setting `comment_strategy` to `one_per_pipeline` adds a single comment for
...
Note: The `one_per_pipeline` and `disable_all` strategy applies only to
