...
Even if you have a custom prefix configured in your Repository CR, both the standard commands (e.g., `/test`) and prefixed commands (e.g., `/pac-test`)will work, so you can still trigger/cancel PipelineRuns with `/test`, `/retest`, and `/cancel` commands.
