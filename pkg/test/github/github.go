...
func SetupGitTree(t *testing.T, mux *http.ServeMux, dir string, event *info.Event, recursive bool) {
	...
	// Shared file map for GraphQL handler - accumulates files from all recursive calls
	var graphQLFileMap graphQLFileMapType
	if !recursive {
		graphQLFileMap = make(graphQLFileMapType)
	}
	if recursive {
	...
	for _, f := range files {
		...
		// Add files to GraphQL map
		if !f.isdir && graphQLFileMap != nil {
			relPath := strings.TrimPrefix(f.name, dir+"/")
			graphQLFileMap[relPath] = f
		}
	}
