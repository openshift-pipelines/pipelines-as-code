...
func TestRemote(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// This test verifies the fix for SRVKP-10604: when multiple PipelineRuns
			// reference the SAME remote pipeline with relative task paths, the second
			// PipelineRun should also resolve the relative paths correctly (using cached pipeline).
			name: "multiple pipelineruns sharing same remote pipeline with relative tasks",
			pipelineruns: []*tektonv1.PipelineRun{
				ttkn.MakePR(randomPipelineRunName, map[string]string{
					apipac.Pipeline: remotePipelineURL,
				},
