...
func getGitOpsCommentPrefix(comment string, repo *v1alpha1.Repository) string {
	if repo.Spec.Settings != nil && repo.Spec.Settings.GitOpsCommandPrefix != "" {
		// to make it compatible with current GitOps commands and allow users to use standard GitOps commands e.g. /test, /retest, /cancel,
		// we allow it to be prefixed with the command prefix.
		prefix := regexp.QuoteMeta(repo.Spec.Settings.GitOpsCommandPrefix)
...
func (v *Provider) handleIssueCommentEvent(ctx context.Context, event *github.IssueCommentEvent) (*info.Event, error) {
	...
	gitOpsCommentPrefix := getGitOpsCommentPrefix(event.GetComment().GetBody(), repo)
