...
type Provider struct {
	...
	credentialsCache   *appCredentialsCache
}

type appCredentialsCache struct {
	mutex       *sync.RWMutex
	credentials map[string]*cachedAppCredentials
}

type cachedAppCredentials struct {
	applicationID int64
	privateKey    []byte
}
...
func New() *Provider {
	return &Provider{
		...
		credentialsCache: &appCredentialsCache{
			mutex:       &sync.RWMutex{},
			credentials: make(map[string]*cachedAppCredentials),
		},
	}
}
