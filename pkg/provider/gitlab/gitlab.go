...
func (v *Provider) createOrUpdateStatusComment(_ context.Context, event *info.Event, commit, updateMarker string, cachedCommentID int64) (int64, error) {
	...
	if updateMarker != "" && cachedCommentID > 0 {
		...
		updatedComment, resp, err := v.Client().Notes.UpdateMergeRequestNote(event.TargetProjectID, event.PullRequestNumber, int(cachedCommentID), &gitlab.UpdateMergeRequestNoteOptions{
			Body: &commit,
		})
...
func (v *Provider) createOrUpdateComment(event *info.Event, commit, updateMarker string) (int64, error) {
		...
		for {
			...
			for _, comment := range comments {
				if commentRe.MatchString(comment.Body) {
					updatedComment, _, err := v.Client().Notes.UpdateMergeRequestNote(event.TargetProjectID, event.PullRequestNumber, comment.ID, &gitlab.UpdateMergeRequestNoteOptions{
						Body: &commit,
					})
					if err != nil {
	...
	createdComment, _, err := v.Client().Notes.CreateMergeRequestNote(event.TargetProjectID, event.PullRequestNumber, &gitlab.CreateMergeRequestNoteOptions{
		Body: &commit,
	})
	if err != nil {
