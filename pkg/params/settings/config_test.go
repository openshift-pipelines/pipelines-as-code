...
func TestSyncConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedError: "failed to parse error detection patterns: invalid regex: error parsing regexp: missing closing ]: `[`",
		},
		{
			name: "multiple patterns via multi-line YAML",
			configMap: map[string]string{
		...
		{
			name: "multiple patterns via JSON array",
			configMap: map[string]string{
				"error-detection-simple-regexp": `["^(?P<filename>[^:]*):(?P<line>[0-9]+):(?P<column>[0-9]+)?([ ]*)?(?P<error>.*)", "^ERROR: (?P<filename>[^ ]+) line (?P<line>[0-9]+): (?P<error>.*)", "^\\[(?P<filename>[^\\]]+)\\]:(?P<line>[0-9]+) - (?P<error>.*)"]`,
			},
			expectedStruct: Settings{
				ApplicationName:                    "Pipelines as Code CI",
				HubCatalogs:                        nil,
