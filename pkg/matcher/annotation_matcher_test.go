...
func TestMatchPipelinerunAnnotationAndRepositories(t *testing.T) {
	...
	tests := []annotationTest{
		...
		{
			name: "cel/custom-params-target-namespace",
			args: annotationTestArgs{
				pruns: []*tektonv1.PipelineRun{
					{
						ObjectMeta: metav1.ObjectMeta{
							Name: "pipeline-default-repo",
							Annotations: map[string]string{
								// This should NOT match because default repo has repo_env="production", not "staging"
								keys.OnCelExpression: "event == \"push\" && repo_env == \"staging\"",
							},
						},
					},
					{
						ObjectMeta: metav1.ObjectMeta{
							Name: "pipeline-target-namespace",
							Annotations: map[string]string{
								keys.TargetNamespace: "other-namespace",
								// This SHOULD match because other-namespace repo has repo_env="staging"
								keys.OnCelExpression: "event == \"push\" && repo_env == \"staging\"",
							},
						},
					},
			...
			// Pipeline with target-namespace should match using params from the targeted repository
			wantPRName: "pipeline-target-namespace",
			wantErr:    false,
		},
		{
