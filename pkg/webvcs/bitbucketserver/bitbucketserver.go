...
func (v *VCS) concatAllYamlFiles(objects []string, runevent *info.Event) (string, error) {
...
func (v *VCS) getRaw(runevent *info.Event, revision string, path string) (string, error) {
...
func (v *VCS) GetTektonDir(ctx context.Context, event *info.Event, path string) (string, error) {
...
func (v *VCS) GetFileInsideRepo(ctx context.Context, event *info.Event, path string, targetBranch string) (string, error) {
	branch := event.SHA
...
func (v *VCS) SetClient(ctx context.Context, opts *info.PacOpts) error {
	if opts.VCSUser == "" {
		return fmt.Errorf("no webvcs_api_user has been set in the repo crd")
	}
	if opts.VCSToken == "" {
		return fmt.Errorf("no webvcs_api_secret has been set in the repo crd")
	}
	if opts.VCSAPIURL == "" {
		return fmt.Errorf("no webvcs_api_url has been set in the repo crd")
	}

	// make sure we have /rest at the end of the url
	if !strings.HasSuffix(opts.VCSAPIURL, "/rest") {
		opts.VCSAPIURL += "/rest"
	}

	// make sure we strip slashes from the end of the URL
	opts.VCSAPIURL = strings.TrimSuffix(opts.VCSAPIURL, "/")
	v.apiURL = opts.VCSAPIURL

	basicAuth := bbv1.BasicAuth{UserName: opts.VCSUser, Password: opts.VCSToken}

	ctx = context.WithValue(ctx, bbv1.ContextBasicAuth, basicAuth)
	cfg := bbv1.NewConfiguration(opts.VCSAPIURL)
